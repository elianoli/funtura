...
class Tabela:
    ...
    def inserir(self, valores_inseridos):
        ...
        valores_inseridos_formatado = []

        self.comando_sql = "INSERT INTO {} {} VALUES {}".format(self.nome, self.parametros[0], self.parametros[1])
        
        for i in range(len(valores_inseridos)):
            valores_inseridos_formatado.append(unidecode(str(valores_inseridos[i]).upper()))

        print("comando sem edição:", valores_inseridos, type(valores_inseridos))
        print("comando com edição:", valores_inseridos_formatado, type(valores_inseridos_formatado))
        self.cursor.execute(self.comando_sql, valores_inseridos_formatado)
        self.banco.commit()
    ...
    def inserir_mov_caixa(self, dataHora,valor,id_caixa,id_tm): #Fução para inserir movimento de caixa quando não tem um IdOS
        ...
        self.cursor.execute(unidecode(self.comando_sql.upper()))
        self.banco.commit()
    ...
    def atualizar(self, valores_alterados):
        ...
        self.comando_sql = "UPDATE {} SET {} WHERE {} = {}".format(self.nome, unidecode(self.conjunto.upper()), self.arquivoTabela["nomes_tabelas"][0], valores_alterados[0])
    ...
    def pagar_parcela_filtro(self, filtro):
        self.comando_sql = ("update parcela set estadoParcela = 'quitada' {}".format(filtro))
        self.cursor.execute(unidecode(self.comando_sql.upper()))
        self.banco.commit()
